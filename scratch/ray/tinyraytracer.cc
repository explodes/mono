...
#include "scratch/ray/geometry.h"
...
Vec3f CastRay(const Vec3f &orig, const Vec3f &dir,
  ...
  Vec3f reflect_color =
      CastRay(reflect_orig, reflect_dir, spheres, lights, depth + 1);
  Vec3f refract_color =
      CastRay(refract_orig, refract_dir, spheres, lights, depth + 1);

  float diffuse_light_intensity = 0, specular_light_intensity = 0;
  for (size_t i = 0; i < lights.size(); i++) {
    Vec3f light_dir = (lights[i].position - point).Normalize();
    float light_distance = (lights[i].position - point).Normal();

    Vec3f shadow_orig =
        light_dir * N < 0 ? point - N * 1e-3
                          : point + N * 1e-3;  // checking if the point lies in
                                               // the shadow of the lights[i]
    Vec3f shadow_pt, shadow_N;
    Material tmpmaterial;
    if (SceneIntersect(shadow_orig, light_dir, spheres, shadow_pt, shadow_N,
                       tmpmaterial) &&
        (shadow_pt - shadow_orig).Normal() < light_distance)
      continue;
  ...
  for (size_t j = 0; j < height; j++) {
    for (size_t i = 0; i < width; i++) {
      ...
      framebuffer[i + j * width] =
          CastRay(Vec3f(0, 0, 0), dir, spheres, lights);
    }
  }
