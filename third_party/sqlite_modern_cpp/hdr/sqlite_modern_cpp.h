...
class database_binder {
 public:
  ...
  std::string sql() {
    ...
    std::unique_ptr<char, decltype(sqlite_deleter)> str(
        sqlite3_expanded_sql(_stmt.get()), sqlite_deleter);
  ...
  std::string original_sql() { return sqlite3_sql(_stmt.get()); }

  void used(bool state) {
    if (execution_started == true && state == true) {
      throw errors::reexecution(
          "Already used statement executed again! Please reset() first!",
          sql());
    }
 ...
 private:
  ...
  void _extract_single_value(std::function<void(void)> call_back) {
    ...
    if ((hresult = sqlite3_step(_stmt.get())) == SQLITE_ROW) {
      ...
      throw errors::no_rows("no rows to extract: exactly 1 row expected", sql(),
                            SQLITE_DONE);
    }
  ...
  sqlite3_stmt* _prepare(const std::u16string& sql) {
    return _prepare(
        std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>, wchar_t>()
            .to_bytes(reinterpret_cast<const wchar_t*>(sql.c_str())));
  }
#else
  sqlite3_stmt* _prepare(const std::u16string& sql) {
    return _prepare(
  ...
  sqlite3_stmt* _prepare(const std::string& sql) {
    ...
    if (!std::all_of(remaining, sql.data() + sql.size(),
                     [](char ch) { return std::isblank(ch); }))
      throw errors::more_statements(
          "Multiple semicolon separated statements are unsupported", sql);
    return tmp;
  }
  ...
  struct is_sqlite_value<std::variant<Args...>>
  ...
  friend database_binder& operator<<(
      database_binder& db, const boost::optional<BoostOptionalT>& val);
  template <typename BoostOptionalT>
  friend void get_col_from_db(database_binder& db, int inx,
                              boost::optional<BoostOptionalT>& o);
#endif

 public:
  database_binder(std::shared_ptr<sqlite3> db, std::u16string const& sql)
      : _db(db), _stmt(_prepare(sql), sqlite3_finalize), _inx(1) {}
  ...
  typename std::enable_if<is_sqlite_value<Result>::value, void>::type
  ...
  typename std::enable_if<!is_sqlite_value<Function>::value, void>::type
...
inline typename std::enable_if<(sizeof...(Values) == Count), void>::type step(
    sqlite3_context* db, int, sqlite3_value**, Values&&... values);
...
inline typename std::enable_if<(sizeof...(Values) < Count), void>::type scalar(
    sqlite3_context* db, int count, sqlite3_value** vals, Values&&... values);
...
class database {
 ...
 public:
  database(const std::string& db_name, const sqlite_config& config = {})
      : _db(nullptr) {
    sqlite3* tmp = nullptr;
    auto ret = sqlite3_open_v2(db_name.data(), &tmp,
                               static_cast<int>(config.flags), config.zVfs);
    ...
    if (ret != SQLITE_OK)
      errors::throw_sqlite_error(_db ? sqlite3_extended_errcode(_db.get())
                                     : ret);
  ...
  database(const std::u16string& db_name, const sqlite_config& config = {})
    ...
    auto db_name_utf8 =
        std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>, wchar_t>()
            .to_bytes(reinterpret_cast<const wchar_t*>(db_name.c_str()));
#else
    auto db_name_utf8 =
        std::wstring_convert<std::codecvt_utf8_utf16<char16_t>, char16_t>()
            .to_bytes(db_name);
#endif
    sqlite3* tmp = nullptr;
    auto ret = sqlite3_open_v2(db_name_utf8.data(), &tmp,
                               static_cast<int>(config.flags), config.zVfs);
    _db = std::shared_ptr<sqlite3>(tmp, [=](sqlite3* ptr) {
      sqlite3_close_v2(ptr);
    });  // this will close the connection eventually when no longer needed.
  ...
  void define(const std::string& name, Function&& func) {
    ...
    if (int result = sqlite3_create_function_v2(
            ...
            sql_function_binder::scalar<
                traits::arity, typename std::remove_reference<Function>::type>,
            nullptr, nullptr,
  ...
  void define(const std::string& name, StepFunction&& step,
              FinalFunction&& final) {
    typedef utility::function_traits<StepFunction> traits;
    using ContextType = typename std::remove_reference<
        typename traits::template argument<0>>::type;

    auto funcPtr = new auto(std::make_pair(std::forward<StepFunction>(step),
                                           std::forward<FinalFunction>(final)));
    if (int result = sqlite3_create_function_v2(
            _db.get(), name.c_str(), traits::arity - 1, SQLITE_UTF8, funcPtr,
            nullptr,
            sql_function_binder::step<
                ContextType, traits::arity,
                typename std::remove_reference<decltype(*funcPtr)>::type>,
            sql_function_binder::final<
                ContextType,
                typename std::remove_reference<decltype(*funcPtr)>::type>,
            [](void* ptr) { delete static_cast<decltype(funcPtr)>(ptr); }))
      errors::throw_sqlite_error(result);
...
inline database_binder& operator<<(database_binder& db,
                                   const sqlite_int64& val) {
  int hresult;
  if ((hresult = sqlite3_bind_int64(db._stmt.get(), db._inx, val)) !=
      SQLITE_OK) {
    errors::throw_sqlite_error(hresult, db.sql());
  }
...
inline database_binder& operator<<(database_binder& db, const float& val) {
  int hresult;
  if ((hresult = sqlite3_bind_double(db._stmt.get(), db._inx, double(val))) !=
      SQLITE_OK) {
    errors::throw_sqlite_error(hresult, db.sql());
  }
...
inline database_binder& operator<<(database_binder& db, const double& val) {
  int hresult;
  if ((hresult = sqlite3_bind_double(db._stmt.get(), db._inx, val)) !=
      SQLITE_OK) {
    errors::throw_sqlite_error(hresult, db.sql());
  }
...
inline database_binder& operator<<(database_binder& db,
  ...
  int hresult;
  if ((hresult = sqlite3_bind_blob(db._stmt.get(), db._inx, buf, bytes,
...
inline void get_col_from_db(database_binder& db, int inx,
  ...
  if (sqlite3_column_type(db._stmt.get(), inx) == SQLITE_NULL) {
    ...
    T const* buf =
        reinterpret_cast<T const*>(sqlite3_column_blob(db._stmt.get(), inx));
    vec = std::vector<T, A>(buf, buf + bytes / sizeof(T));
  }
}
template <typename T, typename A>
inline void get_val_from_db(sqlite3_value* value, std::vector<T, A>& vec) {
  if (sqlite3_value_type(value) == SQLITE_NULL) {
    vec.clear();
  } else {
...
inline database_binder& operator<<(database_binder& db,
                                   const std::string& txt) {
  int hresult;
  if ((hresult = sqlite3_bind_text(db._stmt.get(), db._inx, txt.data(), -1,
                                   SQLITE_TRANSIENT)) != SQLITE_OK) {
    errors::throw_sqlite_error(hresult, db.sql());
  }
...
inline void get_col_from_db(database_binder& db, int inx, std::u16string& w) {
  if (sqlite3_column_type(db._stmt.get(), inx) == SQLITE_NULL) {
    ...
    w = std::u16string(reinterpret_cast<char16_t const*>(
        sqlite3_column_text16(db._stmt.get(), inx)));
  }
}
inline void get_val_from_db(sqlite3_value* value, std::u16string& w) {
  if (sqlite3_value_type(value) == SQLITE_NULL) {
    ...
    w = std::u16string(
        reinterpret_cast<char16_t const*>(sqlite3_value_text16(value)));
  }
}
...
inline database_binder& operator<<(database_binder& db,
                                   const std::u16string& txt) {
  int hresult;
  if ((hresult = sqlite3_bind_text16(db._stmt.get(), db._inx, txt.data(), -1,
                                     SQLITE_TRANSIENT)) != SQLITE_OK) {
    errors::throw_sqlite_error(hresult, db.sql());
  }
...
template <typename BoostOptionalT>
inline database_binder& operator<<(database_binder& db,
                                   const boost::optional<BoostOptionalT>& val) {
  if (val) {
    return operator<<(std::move(db), std::move(*val));
  }
  int hresult;
...
template <typename BoostOptionalT>
inline void get_val_from_db(sqlite3_value* value,
                            boost::optional<BoostOptionalT>& o) {
  if (sqlite3_value_type(value) == SQLITE_NULL) {
    o.reset();
  } else {
...
inline void store_result_in_db(sqlite3_context* db,
  ...
  std::visit(
      [&](auto&& opt) {
        store_result_in_db(db, std::forward<decltype(opt)>(opt));
      },
      val);
}
template <typename... Args>
inline void get_col_from_db(database_binder& db, int inx,
                            std::variant<Args...>& val) {
  utility::variant_select<Args...>(sqlite3_column_type(db._stmt.get(), inx))(
      [&](auto v) {
        get_col_from_db(db, inx, v);
        val = std::move(v);
      });
...
template <typename... Args>
inline void get_val_from_db(sqlite3_value* value, std::variant<Args...>& val) {
...
inline void step(sqlite3_context* db, int count, sqlite3_value** vals) {
  ...
  if (ctxt && ctxt->constructed) ctxt->~AggregateCtxt();
}
